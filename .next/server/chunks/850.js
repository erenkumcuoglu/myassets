"use strict";exports.id=850,exports.ids=[850],exports.modules={7880:(t,e,a)=>{a.a(t,async(t,s)=>{try{a.d(e,{m:()=>n});var r=a(9487),c=t([r]);function i(t){return Number(t.toFixed(2))}async function n(t,e=new Map){let a=await (0,r.zh)(),s=a.find(t=>"USDTRY"===t.pair)?.rate??32,c=a.find(t=>"EURTRY"===t.pair)?.rate??35,n=[...t].sort((t,e)=>t.date.localeCompare(e.date)||t.id-e.id),E=new Map;for(let t of n){let e=E.get(t.assetId)??{assetId:t.assetId,asset:t.asset,lots:[],lastKnownPrice:t.price,realizedPnL:0};if(e.lastKnownPrice=t.price,"BUY"===t.type)e.lots.push({quantity:t.quantity,unitCost:t.price}),E.set(t.assetId,e);else{let a=t.quantity,s=0;if(a>e.lots.reduce((t,e)=>t+e.quantity,0))throw Error(`Cannot sell ${t.quantity} ${t.asset.ticker}; insufficient quantity available.`);let r=e.lots.map(t=>({...t}));for(;a>1e-7;){let e=r[0];if(!e)throw Error(`FIFO inventory exhausted while processing ${t.asset.ticker}.`);let c=Math.min(a,e.quantity);s+=c*e.unitCost,e.quantity-=c,a-=c,e.quantity<=1e-7&&r.shift()}let c=t.quantity*t.price;e.lots=r,e.realizedPnL+=c-s,e.lots.length>0?E.set(t.assetId,e):E.delete(t.assetId)}}let u=Array.from(E.values()).map(t=>{let a=t.lots.reduce((t,e)=>t+e.quantity,0),r=t.lots.reduce((t,e)=>t+e.quantity*e.unitCost,0),n=e.get(t.assetId),E=n?.price??t.lastKnownPrice,u=n?.currency??t.asset.currency,T=E;u!==t.asset.currency&&("TRY"===u&&"USD"===t.asset.currency?T=E/s:"TRY"===u&&"EUR"===t.asset.currency?T=E/c:"USD"===u&&"TRY"===t.asset.currency?T=E*s:"EUR"===u&&"TRY"===t.asset.currency&&(T=E*c));let d=a*T,o=d-r,l=r>0?o/r:0;return{asset:t.asset,totalQuantity:i(a),averageCostBasis:i(a>0?r/a:0),currentPrice:i(T),currentValue:i(d),totalCost:i(r),unrealizedPnL:i(o),unrealizedPnLPercent:l,realizedPnL:i(t.realizedPnL)}}).filter(t=>t.totalQuantity>0).sort((t,e)=>e.currentValue-t.currentValue),T=u.reduce((t,e)=>"TRY"===e.asset.currency?t+e.currentValue:"USD"===e.asset.currency?t+e.currentValue*s:"EUR"===e.asset.currency?t+e.currentValue*c:t,0),d=u.reduce((t,e)=>"TRY"===e.asset.currency?t+e.totalCost:"USD"===e.asset.currency?t+e.totalCost*s:"EUR"===e.asset.currency?t+e.totalCost*c:t,0),o=u.reduce((t,e)=>"TRY"===e.asset.currency?t+e.realizedPnL:"USD"===e.asset.currency?t+e.realizedPnL*s:"EUR"===e.asset.currency?t+e.realizedPnL*c:t,0),l=T-d,N=o+l,R={totalValue:i(T),totalCostBasis:i(d),netInvested:i(d),realizedPnL:i(o),unrealizedPnL:i(l),totalReturn:i(N),totalReturnPercent:d>0?N/d:0};return{positions:u,history:[],summary:R}}r=(c.then?(await c)():c)[0],s()}catch(t){s(t)}})},9487:(t,e,a)=>{a.a(t,async(t,s)=>{try{a.d(e,{FU:()=>w,Hw:()=>p,Ks:()=>Y,Ld:()=>F,QF:()=>I,QW:()=>D,VC:()=>C,Vh:()=>A,aC:()=>P,bE:()=>M,dQ:()=>g,f1:()=>S,initDb:()=>N,mo:()=>q,rw:()=>x,uZ:()=>m,yX:()=>O,zh:()=>h,zm:()=>f});var r=a(7561),c=a.n(r),i=a(9411),n=a.n(i),E=a(4629),u=a(7880),T=t([E,u]);[E,u]=T.then?(await T)():T;let b=n().join(process.cwd(),"data");function d(){return global.portfolioDb||(global.portfolioDb=(0,E.createClient)({url:process.env.DATABASE_URL||"file:/app/data/portfolio.db"})),global.portfolioDb}n().join(b,"portfolio.db"),c().existsSync(b)||c().mkdirSync(b,{recursive:!0});let X=null;async function o(){if(global.schemaInitialized)return;c().existsSync(b)||c().mkdirSync(b,{recursive:!0});let t=d();await t.execute(`
  ...
  `),global.schemaInitialized=!0}async function l(){X||(X=o()),await X}async function N(){let t=n().join(process.cwd(),"data");c().existsSync(t)||c().mkdirSync(t,{recursive:!0});let e=d();await e.execute(`CREATE TABLE IF NOT EXISTS assets (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    fetched_at DATETIME DEFAULT CURRENT_TIMESTAMP
         ...
         ORDER BY t.date DESC, t.id DESC`})).rows.map(t=>({...y(t),asset:{id:Number(t.joined_asset_id),ticker:String(t.ticker),name:String(t.name),assetClass:t.asset_class,currency:t.asset_currency,createdAt:String(t.asset_created_at)}}))}async function p(t){await l();let e=d(),a=await e.execute({sql:`INSERT INTO assets (ticker, name, asset_class, currency)
          ...
          FROM assets
          WHERE id = ?`,args:[s]});return R(r.rows[0])}async function I(t){await l();let e=d();if(!(await e.execute({sql:`SELECT id, ticker, name, asset_class, currency, created_at
          FROM assets
         ...
         FROM transactions t
         INNER JOIN assets a ON a.id = t.asset_id
         WHERE t.id = ?`,args:[s]})).rows[0];return{...y(r),asset:{id:Number(r.joined_asset_id),ticker:String(r.ticker),name:String(r.name),assetClass:r.asset_class,currency:r.asset_currency,createdAt:String(r.asset_created_at)}}}async function w(){let t=await S(),e=await U();return await (0,u.m)(t,e)}async function U(){await l();let t=d(),e=await t.execute({sql:`SELECT pc.id, pc.asset_id, pc.price, pc.currency, pc.fetched_at
          FROM price_cache pc
          INNER JOIN (
           ...
           AND latest.max_fetched_at = pc.fetched_at`});return new Map(e.rows.map(t=>{let e=L(t);return[e.assetId,e]}))}async function C(t){await l();let e=d(),a=await e.execute({sql:`SELECT id, asset_id, price, currency, fetched_at
          FROM price_cache
          WHERE asset_id = ?
          ORDER BY fetched_at DESC, id DESC
          LIMIT 1`,args:[t]});return 0===a.rows.length?null:L(a.rows[0])}async function O(t,e,a){await l();let s=d();await s.execute({sql:`INSERT INTO price_cache (asset_id, price, currency, fetched_at)
          VALUES (?, ?, ?, datetime('now'))`,args:[t,e,a]})}async function f(){await l();let t=d();await t.execute({sql:"DELETE FROM price_cache"})}async function M(){await l();let t=d();await t.execute({sql:"UPDATE assets SET currency = 'TRY' WHERE asset_class = 'COMMODITY'"})}async function h(){await l();let t=d();return(await t.execute({sql:`SELECT id, pair, rate, fetched_at
          FROM fx_rates
          ORDER BY pair ASC`})).rows.map(_)}async function D(t){await l();let e=d(),a=await e.execute({sql:`SELECT id, pair, rate, fetched_at
          FROM fx_rates
          WHERE pair = ?
          ORDER BY fetched_at DESC, id DESC
          LIMIT 1`,args:[t]});return 0===a.rows.length?null:_(a.rows[0])}async function m(t,e){await l();let a=d(),s=await a.execute({sql:`INSERT INTO fx_rates (pair, rate)
         ...
         FROM transactions t
         INNER JOIN assets a ON a.id = t.asset_id
         WHERE t.id = ?`,args:[t]})).rows[0];return{...y(c),asset:{id:Number(c.joined_asset_id),ticker:String(c.ticker),name:String(c.name),assetClass:c.asset_class,currency:c.asset_currency,createdAt:String(c.asset_created_at)}}}async function Y(t){await l();let e=d();await e.execute({sql:"DELETE FROM transactions WHERE id = ?",args:[t]})}async function P(t){await l();let e=d(),a=await e.execute({sql:`SELECT COALESCE(SUM(CASE WHEN type = 'BUY' THEN quantity ELSE -quantity END), 0) as quantity
          FROM transactions
